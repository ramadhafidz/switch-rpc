...
/// requiring Eden or Discord to be running, printing the metrics that
/// feed the Phase 1 benchmark (poc/BENCHMARKS.md).
/// </summary>
public static class Diagnose
{
	public static int Run()
	{
		Console.WriteLine("SWITCH RPC POC — diagnose");
		Console.WriteLine();
	...
	private static void PrintSaveMetrics()
	{
		...
		foreach (var definition in GameCatalog.Games)
		{
			...
			Console.WriteLine(
				$"  PKHeX identification: {save.GetType().Name}"
			...
			stopwatch.Restart();
			var state = reader.Read(save, definition.Id);
			stopwatch.Stop();
